...
from explainer.utils import get_git_hash, data_dir
...
class ExplainedModel():
    ...
    def __init__(self, model_name=None, dataset=None, data=None, labels=None,
        ...
        if model_name is not None:
            ...
            self.is_loaded = True
        self.model_dir = os.path.join(data_dir, 'model_artifacts', self.model_name)
    ...
    def explain_df_class(self, df):
        X = self.categoricalencoder.transform(df)
        probability = self.pipeline.predict_proba(X)[0,1]
        class_pred = self.pipeline.predict(X)[0]
        e = self.explainer.explain_instance(X[0], 
                                            self.pipeline.predict_proba
                                           ).as_map()[1]
        explanations = {self.explainer.feature_names[c]: weight
                        for c, weight in e}
        return probability, class_pred, explanations

    def explain_dct(self, dct):
        return self.explain_df(pd.DataFrame([dct]))

    def explain_dct_class(self, dct):
        return self.explain_df_class(pd.DataFrame([dct]))
    ...
    def non_categorical_features(self):
        return list(self.data.select_dtypes(exclude=['category']).columns
                    .drop(self.labels.name + ' prediction'))
    ...
    @property
    def label_name(self):
        return self.labels.name + ' prediction'
